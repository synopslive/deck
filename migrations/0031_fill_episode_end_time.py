...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for episode in orm.Episode.objects.filter(end_time=None):
            average_duration = episode.show.average_duration
            try:
                next_episode = episode.get_next_by_time()
                next_start = next_episode.time
            except orm.Episode.DoesNotExist:
                next_start = datetime.datetime.now()

            episode.end_time = min(episode.time + datetime.timedelta(minutes=average_duration), next_start)
            episode.save()
